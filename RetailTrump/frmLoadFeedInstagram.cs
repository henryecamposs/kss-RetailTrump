...
namespace RetailTrump
{
    ...
    public partial class frmLoadFeedInstagram : Form
    {
        FeedInstagram_Collection FeedsInstagram;
        public frmLoadFeedInstagram(FeedInstagram_Collection feeds)
        {
            InitializeComponent();
            FeedsInstagram = feeds;
        }

        private void frmLoadFeedInstagram_Load(object sender, System.EventArgs e)
        {
            ...
            foreach (FeedInstagram item in FeedsInstagram)
            {
                try
                {
                    this.imageList1.Images.Add(LoadPicture(item.Display_url));
                    List<string> str = Program.WrapText(item.Edge_media_to_caption, 250, "Arial", 10);
                    string str1 = string.Join("\n", str);
                    //listViewCustom viewCustom = new listViewCustom(imageList1.Images[i], str1);
                    listView1.Items.Add(str1, i);
                    i += 1;
                }
                catch (Exception)
                {
        ...
        {
            Image img = imageList1.Images[listView1.SelectedItems[0].ImageIndex];
        ...
        {
            Program.Usuarioinstagram = textBox1.Text;
            Program.ConsultaMonitorve();
        }

        private void button1_Click_1(object sender, EventArgs e)
        {
            if ((int)listView1.View <= 3)
                listView1.View += 1;
            else
